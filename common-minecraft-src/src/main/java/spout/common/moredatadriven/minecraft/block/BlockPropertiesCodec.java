...
import spout.common.moredatadriven.minecraft.common.subtypes.BlockStateFunction;
...
public class BlockPropertiesCodec {
    ...
    public static final Codec<BlockBehaviour.Properties> CODEC = new Codec<>() {
        ...
        public <T> DataResult<T> encode(BlockBehaviour.Properties input, DynamicOps<T> ops, T prefix) {
            Block block = Objects.requireNonNull(ContextAwareBlockPropertiesEncoding.getBlock());
            RecordBuilder<T> builder = ops.mapBuilder();
            builder.add("map_color", BlockStateFunction.wrap(block, input.mapColor), SubtypeCodecs.MAP_COLOR_FUNCTION_CODEC);
            ...
            builder.add("light_emission", BlockStateFunction.wrap(block, input.lightEmission), SubtypeCodecs.LIGHT_EMISSION_CODEC);
        ...
        public <T> DataResult<Pair<BlockBehaviour.Properties, T>> decode(DynamicOps<T> ops, T input) {
            return ops.getMap(input).flatMap(mapLike -> {
                BlockBehaviour.Properties properties = NewContextAwareBlockProperties.create();
                ...
                if (lightEmissionInput != null) {
                    ...
                    properties.lightEmission = BlockStateFunction.asToIntFunction(lightEmission.getOrThrow());
                }
                ...
                if (isRedstoneConductorInput != null) {
                    DataResult<BlockBehaviour.StatePredicate> isRedstoneConductor = KnownStatePredicate.CODEC.decode(ops, isRedstoneConductorInput).map(Pair::getFirst);
                ...
                if (isSuffocatingInput != null) {
                    DataResult<BlockBehaviour.StatePredicate> isSuffocating = KnownStatePredicate.CODEC.decode(ops, isSuffocatingInput).map(Pair::getFirst);
                ...
                if (offsetFunctionInput != null) {
                    DataResult<BlockBehaviour.OffsetFunction> offsetFunction = SubtypeCodecs.OFFSET_FUNCTION_CODEC.decode(ops, offsetFunctionInput).map(Pair::getFirst);
    ...
    public static <B extends Block> RecordCodecBuilder<B, BlockBehaviour.Properties> getBuilder() {
        return CodecUtil.optionalFieldOf(CODEC,"properties", NewContextAwareBlockProperties::create).forGetter(BlockBehaviour::properties); // Spout - More data-driven - Minecraft registries - Types - Adapt existing block type registry - Make properties optional in block codec
    }
